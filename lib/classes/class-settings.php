  ...
  if( !class_exists( 'wpCloud\StatelessMedia\Settings' ) ) {
    ...
    final class Settings extends \UsabilityDynamics\Settings {
      ...
      public function sm_fields_advanced_callback() {
        ...
        // use domain as static host
        $inputs[] = '<input type="hidden" name="sm[static_host]" value="false" />';
        $inputs[] = '<label for="sm_body_rewrite"><input id="sm_body_rewrite" type="checkbox" name="sm[static_host]" value="true" '. checked( 'true', $this->get( 'sm.static_host' ), false ) .'/>'.__( 'Use bucket as static host.', ud_get_stateless_media()->domain ).'</label>';
